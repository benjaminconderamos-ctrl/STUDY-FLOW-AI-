...
const MathSolverResult = dynamic(
  () =>
    import("@/components/dashboard/MathSolverResult").then(
      (mod) => mod.MathSolverResult
    ),
  {
    loading: () => (
      <div className="space-y-4 animate-pulse">
        ...
        <div className="h-40 bg-muted rounded-[14px]" />
      </div>
    ),
  }
);
...
export function MathSolverWorkspace({
  initialHistory,
}: {
  initialHistory: MathSolverRequest[];
}) {
  const [result, setResult] = useState<MathResult | null>(null);
  const [currentProblem, setCurrentProblem] = useState("");
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState("");
  const [history, setHistory] = useState<MathSolverRequest[]>(initialHistory);
