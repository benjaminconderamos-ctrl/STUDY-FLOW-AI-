...
export function NewSessionForm({
  ...
  async function handleCreateFromTopic() {
    ...
    if (!trimmedStudyTopic) {
      setError("Describe el tema que quieres estudiar.");
      setLoading(false);
      return;
    }
    ...
    if (trimmedStudyTopic.length > 2000) {
      setError("El tema de estudio no puede exceder 2,000 caracteres.");
      setLoading(false);
      return;
    }
  ...
  const canSubmit =
    !loading &&
    (mode === "topic"
      ? title.trim().length > 0 && studyTopic.trim().length > 0
      : selectedFile !== null);
  ...
  return (
    <PageShell>
      <PageHeader
        ...
        subtitle={
          mode === "topic"
            ? "Ponle un título a tu sesión y describe el tema que quieres estudiar con IA."
            ...
            <div className="space-y-6">
              <div className="space-y-2">
                <label
                  ...
                  className="text-[12px] font-semibold tracking-[0.08em] uppercase text-foreground-muted font-sans"
                >
                  Título de la sesión
                </label>
                <input
                ...
                <p className="text-[11px] text-foreground-muted font-sans">
                  Este nombre se mostrará en tus sesiones.
                </p>
                {title.length > 260 && (
              ...
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  placeholder="Ej. Las tres leyes de Newton, sus fórmulas y ejemplos cotidianos"
                  maxLength={2000}
                  rows={4}
                  className="w-full min-h-28 resize-y px-4 py-3 rounded-[10px] border border-border bg-background text-[15px] text-foreground placeholder:text-foreground-muted font-sans outline-none focus:border-foreground transition-colors duration-150"
                  required
                />
                <div className="flex items-start justify-between gap-4">
                  <p className="text-[11px] text-foreground-muted font-sans">
                    La IA usará este contenido para generar el resumen, quiz, tarjetas y tutor.
                  </p>
                  {studyTopic.length > 1700 && (
                    <p className="text-[11px] text-foreground-muted font-sans whitespace-nowrap">
                      {studyTopic.length}/2000
                    </p>
                  )}
                </div>
              </div>
